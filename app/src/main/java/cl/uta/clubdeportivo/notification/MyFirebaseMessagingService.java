package cl.uta.clubdeportivo.notification;
...
import cl.uta.clubdeportivo.activity.PostDetailsActivity;
...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        if (remoteMessage.getData().size() > 0) {
            ...
            Log.d("Club Deportivo notific", params.toString());

            if (AppPreference.getInstance(MyFirebaseMessagingService.this).isNotificationOn()) {

                sendNotification(params.get("title"), params.get("message"), params.get("postID"));
    ...
    private void sendNotification(String title, String messageBody, String postId) {
        ...
        try {
            ...
            Log.d("ID NOTIF", "sendNotification: " + id);
        } catch (Exception e) {
            e.printStackTrace();
        }

        NotificationManager notificationManager = (NotificationManager) getSystemService(Context.NOTIFICATION_SERVICE);
        String CHANNEL_ID = "ClubDeportivo";
